...
ROUTE = ROOT / "app/api/projectos/memory/search/route.ts"
WORKFLOW = ROOT / ".github/workflows/m5-task-aware-retrieval.yml"
...
def main():
    ...
    if not (failure and procedure and fact):
        raise AssertionError("class priority weights missing")
    if not (int(failure.group(1))>int(procedure.group(1))>int(fact.group(1))):
        raise AssertionError("failure lesson priority invalid")

    for token in (
      'const M5_TYPED_MEMORY_CLASSES','"memory_task_context_v1"','m5_task_aware_bounded',
      'project_scoped_keyword_recency_legacy_until_m5_class_grant','retrieval_does_not_grant_authority',
      'allowed_typed_classes'
    ):
        need(bridge,token)
    for value in sorted(INTENTS):
        need(bridge,f'"{value}"')
    for token in ('"intent"','"action_mode"','"consequential"','"required_capabilities"'):
        need(route,token)
    need(workflow,"app/api/projectos/memory/search/route.ts")

    for phrase in (
      "Production grants are **not** expanded","policy/advisory separation",
      "bounded byte output","cross-project exclusion"
    ):
        need(doc,phrase)
