...
assert(actualFiles.length === 65, `expected 65 migration files, got ${actualFiles.length}`);
assert(evidence.sourceStateAfterRecovery.migrationFiles === 65, "evidence migration file count stale");
assert(evidence.sourceStateAfterRecovery.exactAppliedFiles === 65, "exact applied source count stale");
assert(evidence.sourceStateAfterRecovery.missingAppliedFiles === 20, "missing applied count stale");
...
assert(boundary.recoveredCandidateSafeCount === 32, "recovered legacy-safe count must be 32");
assert(boundary.remainingCandidateSafeCount === 8, "remaining legacy-safe count must be 8");
...
assert(boundary.recoveredManifestBytes === 3841, "recovered manifest bytes changed");
assert(boundary.recoveredManifestSha256 === "88632ac6c00b5a092abc0e3d7262a87d6281ae71cc244790d043bc028dc4e36e", "recovered manifest digest changed");
assert(boundary.remainingSafeManifestBytes === 936, "remaining-safe manifest bytes changed");
assert(boundary.remainingSafeManifestSha256 === "cf3e3f7a2b8e1799c0c486ca8457b1901547c35cf4916bbfc256f49bc3ef2c95", "remaining-safe manifest digest changed");
...
assert(boundary.recoveredSafe.length === 32, "recovered legacy row count changed");
...
assert(expectedFiles.size === 65, `expected source set has ${expectedFiles.size} files`);
...
assert(evidence.recovery.recoveredLegacySafeFiles === 32, "legacy recovery count stale");
assert(evidence.recovery.remainingLegacyFiles === 20, "remaining legacy count stale");
assert(evidence.recovery.remainingCandidateSafeFiles === 8, "remaining safe count stale");
