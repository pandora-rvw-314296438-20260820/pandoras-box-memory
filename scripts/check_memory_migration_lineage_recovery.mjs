...
assert(actualFiles.length === 60, `expected 60 migration files, got ${actualFiles.length}`);
assert(evidence.sourceStateAfterRecovery.migrationFiles === 60, "evidence migration file count stale");
assert(evidence.sourceStateAfterRecovery.exactAppliedFiles === 60, "exact applied source count stale");
assert(evidence.sourceStateAfterRecovery.missingAppliedFiles === 25, "missing applied count stale");
...
assert(boundary.recoveredCandidateSafeCount === 27, "recovered legacy-safe count must be 27");
assert(boundary.remainingCandidateSafeCount === 13, "remaining legacy-safe count must be 13");
...
assert(boundary.recoveredManifestBytes === 3264, "recovered manifest bytes changed");
assert(boundary.recoveredManifestSha256 === "1e2afc954228338fc78144367a9ff84951c6e583283f0adf376adb0ad1969986", "recovered manifest digest changed");
assert(boundary.remainingSafeManifestBytes === 1513, "remaining-safe manifest bytes changed");
assert(boundary.remainingSafeManifestSha256 === "d530c2c268d771ae2973de0a485c53bd5239d3a31cbd5d6f0a647e145c1184df", "remaining-safe manifest digest changed");
...
assert(boundary.recoveredSafe.length === 27, "recovered legacy row count changed");
...
assert(expectedFiles.size === 60, `expected source set has ${expectedFiles.size} files`);
...
assert(evidence.recovery.recoveredLegacySafeFiles === 27, "legacy recovery count stale");
assert(evidence.recovery.remainingLegacyFiles === 25, "remaining legacy count stale");
assert(evidence.recovery.remainingCandidateSafeFiles === 13, "remaining safe count stale");
...
process.stdout.write("Memory migration lineage checkpoint valid: 60/85 exact source; legacy boundary 40 safe / 12 quarantined; 27 safe restored / 13 safe pending.\n");
