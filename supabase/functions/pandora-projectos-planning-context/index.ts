...
const uuid = (value: unknown): string | null => {
  ...
  return UUID.test(valueText) ? valueText : null;
};
const hash = (value: unknown): string | null => {
  const valueText = text(value).toLowerCase();
  return HASH.test(valueText) ? valueText : null;
};
...
const queryBasis = (input: {
  organizationId: string; visibleProjectId: string; decisionType: string;
}): string => [
  "projectos-planning-query-v1",
  input.organizationId,
  input.visibleProjectId,
  MEMORY_PROJECT_ID,
  MEMORY_PROJECT_KEY,
  input.decisionType,
].join("\n");
const requestBasis = (input: {
  requestId: string; organizationId: string; visibleProjectId: string; decisionType: string; queryHash: string;
}): string => [
  PURPOSE,
  input.requestId,
  input.organizationId,
...
const responseBasis = (input: {
  requestId: string; organizationId: string; visibleProjectId: string; decisionType: string; queryHash: string;
  retrievalLogId: string; contextStatus: string; approvedMemoryItemIds: string[]; highlights: JsonRecord[];
}): string => [
  "projectos-planning-context-response-v1",
  input.requestId,
  input.organizationId,
  input.visibleProjectId,
  MEMORY_PROJECT_ID,
  ...
  input.contextStatus,
  input.approvedMemoryItemIds.join(","),
  ...input.highlights.map((item) => [
    text(item.id), text(item.memory_type), compact(item.summary), text(item.updated_at),
  ].join("|")),
].join("\n");
...
Deno.serve(async (request: Request) => {
  ...
  try { payload = record(JSON.parse(raw)); } catch { return json(400, { ok: false, error: "invalid_json" }); }
  if (!exactKeys(payload, [
    "schema_version", "purpose", "request_id", "organization_id", "visible_project_id",
    "memory_project_id", "project_key", "decision_type", "query_hash",
  ...
  const memoryProjectId = uuid(payload.memory_project_id);
  ...
  if (payload.schema_version !== 1 || payload.purpose !== PURPOSE || !requestId || !organizationId || !visibleProjectId
      || memoryProjectId !== MEMORY_PROJECT_ID || projectKey !== MEMORY_PROJECT_KEY
      || !DECISION_TYPES.has(decisionType) || !queryHash) {
    return json(400, { ok: false, error: "planning_identity_invalid" });
  }
  const expectedQueryHash = await sha256(queryBasis({ organizationId, visibleProjectId, decisionType }));
  ...
  const expectedSignature = await hmacHex(secret, `${timestamp}.${requestBasis({ requestId, organizationId, visibleProjectId, decisionType, queryHash })}`);
  ...
  const { data: project, error: projectError } = await admin
    ...
    .eq("id", MEMORY_PROJECT_ID)
    .eq("project_key", MEMORY_PROJECT_KEY)
    .eq("memory_namespace", NAMESPACE)
    .eq("lifecycle_status", "active")
    .maybeSingle();
  if (projectError || !project) return json(503, { ok: false, error: "memory_project_unavailable" });

  const { data: claimed, error: claimError } = await admin.rpc("memory_claim_projectos_planning_nonce_v1", {
    p_request_id: requestId,
    p_organization_id: organizationId,
    p_visible_project_id: visibleProjectId,
    p_memory_project_id: MEMORY_PROJECT_ID,
    p_project_key: MEMORY_PROJECT_KEY,
  ...
  const { data: items, error: itemError } = await admin
    ...
    .eq("project_id", MEMORY_PROJECT_ID)
    .eq("is_active", true)
    .in("canon_status", APPROVED_CANON)
  ...
  for (const rawItem of items ?? []) {
    ...
    if (!id) continue;
    const title = compact(item.title, 220);
    const sourceSummary = compact(item.source_summary, 420);
    const summary = compact([title, sourceSummary].filter(Boolean).join(": "));
    if (!summary || SECRETISH.test(summary)) continue;
    highlights.push({
      id,
      memory_type: compact(item.memory_type, 80),
      summary,
      updated_at: compact(item.updated_at, 64),
    });
  }
  ...
  const { data: retrievalLog, error: retrievalError } = await admin
    ...
    .insert({
      ...
      metadata: {
        principal: "projectos-hmac-planning",
        purpose: PURPOSE,
        ...
        returned_approved_items: approvedMemoryItemIds.length,
        summary_source: "title_plus_source_summary_only",
  ...
  const contextHash = await sha256(responseBasis({
    requestId, organizationId, visibleProjectId, decisionType, queryHash,
    retrievalLogId, contextStatus, approvedMemoryItemIds, highlights,
  }));
  return json(200, {
    ok: true,
    schema_version: 1,
