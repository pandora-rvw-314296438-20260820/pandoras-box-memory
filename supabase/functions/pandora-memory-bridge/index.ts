...
const warningsFor = (input: {
  ...
  if (input.approvedCount === 0) {
    warnings.push(
      "No approved Memory records were returned for the Pandora service principal.",
    );
  }
  if (
...
const searchMemory = async (
  ...
  if (boundProjectError) {
    console.error("pandora_memory_project_lookup_failed", boundProjectError.message);
  ...
  if (projectGrantError) {
    console.error("pandora_memory_project_grant_lookup_failed", projectGrantError.message);
  ...
  if (contextPackError) {
    console.error("pandora_memory_context_pack_failed", {
  ...
  if (allowedTypedClasses.length > 0) {
    ...
    if (typedContextError) {
      console.error("pandora_memory_task_context_failed", {
    ...
    const { data: indexedItems, error: indexedItemsError } = await supabase.rpc(
      "memory_pandora_search_scoped_v1",
      {
        p_user_id: principal.memory_user_id,
        p_namespace: namespace,
        p_project_id: canonicalProjectId,
    ...
    if (indexedItemsError) {
      console.error("pandora_memory_search_failed", {
  ...
  const { data: retrievalLog, error: logError } = await supabase
    ...
    .maybeSingle();
  if (logError) {
    console.error("pandora_memory_retrieval_log_failed", {
      code: logError.code,
  ...
  return respond({
    ...
    retrieval_reasoning_summary: retrievalMode === "m5_task_aware_bounded"
      ? "Pandora received only task-relevant, bounded, typed Memory already permitted by the exact project grant. Policy Memory is separated from advisory Memory and retrieval does not grant execution authority."
      : "Pandora retained the existing exact-project keyword/recency retrieval because this grant does not yet authorize M5 typed classes; no grant expansion was inferred.",
    warnings,
  });
};
...
const EVIDENCE_SOURCE = "pandora-post-task";
const EVIDENCE_CANDIDATE_TYPE = "pandora_outcome";
const EVIDENCE_INTAKE_KIND = "pandora_evidence_candidate_v1";
...
const ensureEvidenceReviewItem = async (
  ...
  if (existingReviewError) {
    console.error("pandora_evidence_review_lookup_failed", existingReviewError.message);
  ...
  if (reviewInsertError && errorCode(reviewInsertError) !== "23505") {
    console.error("pandora_evidence_review_insert_failed", reviewInsertError.message);
...
const submitEvidenceCandidate = async (
  ...
  const { data: boundProject, error: boundProjectError } = await projectQuery.maybeSingle();
  if (boundProjectError) {
    console.error("pandora_evidence_project_lookup_failed", boundProjectError.message);
  ...
  const { data: projectGrant, error: projectGrantError } = await admin
    ...
    .maybeSingle();
  if (projectGrantError) {
    console.error("pandora_evidence_project_grant_lookup_failed", projectGrantError.message);
  ...
  const sourceRef = `pandora-evidence:${canonicalProjectId}:${idempotencyKey}`;
  ...
  if (existingCandidateError) {
    console.error("pandora_evidence_candidate_lookup_failed", existingCandidateError.message);
  ...
  if (existingCandidate?.id) {
    ...
    if (!persistedSnapshot) {
      console.error("pandora_evidence_candidate_unreadable", sourceRef);
  ...
  if (!candidateId) {
    const candidate = {
      ...
      status: "pending",
      reason:
        "Pandora evidence intake is review-gated. This candidate cannot become canonical without an authenticated human decision.",
      people: [],
      projects: [projectReference],
      risks: [],
      tags: ["pandora", "evidence_candidate", evidenceKind, proofStage],
      metadata: {
        schema_version: 1,
      ...
      stale_status: "active",
      scoring_version: "pandora-evidence-v1",
      scored_at: now,
    };
    ...
    if (candidateInsertError && errorCode(candidateInsertError) !== "23505") {
      console.error("pandora_evidence_candidate_insert_failed", candidateInsertError.message);
